...
// ✅ 1️⃣ Récupérer les statistiques de ventes
router.get('/', async (req, res) => {
    try {
        const [rows] = await db.promise().query(`
            SELECT * FROM statistiques_ventes LIMIT 1
        `);
        res.json(rows[0]);
    } catch (err) {
...
// ✅ 2️⃣ Mettre à jour les statistiques de ventes
router.put('/update', async (req, res) => {
    try {
        ...
        const [updatedStats] = await db.promise().query('SELECT * FROM statistiques_ventes LIMIT 1');

        res.status(200).json({ message: 'Statistiques mises à jour avec succès', data: updatedStats[0] });
    } catch (err) {
