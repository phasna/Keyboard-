...
const UserList = () => {
    ...
    return (
        <div className="w-screen mx-auto text-white p-4 md:p-5 relative pb-20">
            <motion.h2
                className={`text-3xl md:text-5xl font-light mb-6 md:mb-10 text-center md:text-left ${containerBlur}`}
            ...
            <div className={`mb-4 relative ${containerBlur}`}>
                <div className={`relative flex items-center w-full ${isSearchFocused ? 'ring-2 ring-blue-500' : ''}`}>
            ...
            {/* Affichage bureau */}
            <div className={`hidden md:block overflow-x-auto shadow-2xl rounded-xl ${containerBlur}`}>
                <table className="w-full border-collapse bg-black bg-opacity-40 rounded-xl text-white">
                    <thead>
                    ...
                    {filteredUsers.length === 0 ? (
                        ...
                        filteredUsers.map(user => (
                            <motion.div
                                ...
                                exit={{ opacity: 0, y: -20 }}
                                transition={{ duration: 0.3 }}
                                className="mb-4 bg-black bg-opacity-40 rounded-xl overflow-hidden shadow-lg border border-gray-800"
                            >
                                <div
                                            ...
                                            <div className="p-4 space-y-2">
                                                ...
                                                <div className="flex justify-end space-x-3 pt-3">
                                                    <motion.button
                                                        ...
                                                        whileTap={{ scale: 0.95 }}
                                                    >
                                                        <FaEdit size={16} className="mr-1 inline" /> Modifier
                                                    </motion.button>
                                                    <motion.button
                                                        onClick={() => handleDelete(user.id)}
                                                        ...
                                                        whileTap={{ scale: 0.95 }}
                                                    >
                                                        <FaTrash size={16} className="mr-1 inline" /> Supprimer
                                                    </motion.button>
                                                </div>
                                            </div>
                                        </motion.div>
            ...
            {/* Modal d'édition - Amélioré pour mobile */}
            <AnimatePresence>
                {showEditModal && (
                    <motion.div
                    ...
                    >
                        <motion.div
                            className="bg-zinc-800 rounded-xl w-11/12 sm:w-4/5 md:w-2/3 lg:w-1/2 p-4 max-h-[90vh] overflow-y-auto"
                            initial={{ scale: 0.9, y: 20 }}
                            animate={{ scale: 1, y: 0 }}
                            exit={{ scale: 0.9, y: 20 }}
                        >
                            <div className="flex justify-between items-center mb-6 sticky top-0 bg-zinc-800 py-2 border-b border-gray-700">
                                <h2 className="text-2xl font-medium">Modifier l'utilisateur</h2>
                                <button
                                ...
                                {selectedUser && (
                                    <>
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            ...
                                            <div className="mb-1">
                                                <label className="block mb-1 text-gray-300 text-sm">Mot de passe</label>
                                                <input
                                                    type="password"
                                                    name="mot_de_passe"
                                            ...
                                            <div className="mb-1">
                                                <label className="block mb-1 text-gray-300 text-sm">URL de l'image</label>
                                                <input
                                                    type="text"
                                                    name="image"
                                        ...
                                        <div className="flex flex-col sm:flex-row justify-end gap-3 mt-6">
                                            <button
                                                onClick={handleModalClose}
                                                className="px-4 py-2 bg-gray-700 rounded-lg text-white order-2 sm:order-1"
                                            >
                                                Annuler
                                            </button>
                                            <button
                                                onClick={handleFormSubmit}
                                                className="px-4 py-2 bg-green-600 text-white rounded-lg order-1 sm:order-2"
                                            >
                                                Enregistrer
                                            </button>
                                        </div>
