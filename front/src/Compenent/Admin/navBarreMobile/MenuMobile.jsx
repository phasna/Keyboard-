...
import { FaPlus, FaEdit, FaUserPlus, FaUserCircle, FaHome, FaUsers, FaMoon } from "react-icons/fa"; // Import icons
...
const FooterMenu = () => {
    ...
    const navItems = [
        ...
        { path: "/Liste_des_commande", icon: <FaUsers />, title: "Liste des commandes" },
        { path: "/accueiladmin", icon: <FaHome />, title: "Accueil" },
        { path: "/listeutilisateurs", icon: <FaUserCircle />, title: "Liste des utilisateurs" },
        { path: "/adduser", icon: <FaUserPlus />, title: "Ajouter un utilisateur" },
        { path: "/updateuser/:id", icon: <FaEdit />, title: "Modifier un utilisateur" },
    ];

    // Determine the active tab index based on the current route
    const activeIndex = navItems.findIndex((item) =>
        item.path.includes(":id") ? location.pathname.startsWith(item.path.split(":")[0]) : location.pathname === item.path
    );
    ...
    return (
        <div className="fixed bottom-0 left-0 w-full bg-gray-900/90 text-white p-4 flex justify-center items-center shadow-2xl rounded-t-3xl z-10">
            {/* Logo (centered above the navigation bar on mobile) */}
            <Link to="/" className="absolute top-[-60px] left-1/2 transform -translate-x-1/2 hidden sm:block">
            ...
            <div className="relative flex justify-around items-center w-full max-w-md">
                ...
                <div
                    ref={highlightRef}
                    className="absolute w-14 h-14 bg-green-500 rounded-full transition-all duration-500 ease-in-out transform -translate-x-1/2 top-1/2 -translate-y-1/2"
                ></div>
                ...
                {navItems.map((item, index) => (
                    <Link
                        ...
                        className={`relative z-10 p-2 transition-all duration-300 ${
                            activeIndex === index ? "text-white scale-125" : "text-gray-400 hover:text-gray-200"
                        }`}
                        title={item.title}
                    >
                        {React.cloneElement(item.icon, {
                            className: `text-xl sm:text-2xl ${activeIndex === index ? "text-3xl" : ""}`,
                        })}
                    </Link>
                ))}
            </div>
