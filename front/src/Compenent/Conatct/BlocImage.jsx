      ...
      <div className="gap-2 grid grid-cols-12 grid-rows-6 px-8">
        ...
        {specialCards.map(({ title, subtitle, image, span, footer }, index) => (
          <motion.div
            ...
            transition={{
              duration: 0.5,
              delay: (index + cardsData.length) * 0.2,
            }}
          ...
          >
            <Card isFooterBlurred className="w-full h-full">
              ...
              <CardFooter
                className={`absolute ${
                  title === "New" ? "bg-white/30" : "bg-black/40"
                } bottom-0 z-10 border-t-1 border-default-600 dark:border-default-100`}
              >
                {footer}
              </CardFooter>
            </Card>
